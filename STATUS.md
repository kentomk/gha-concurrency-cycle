...
- `2026-07-27T12:24:00Z`: 全6 managed repositoryのpublic main CI、Issue／PR、release assetとaggregate trafficをbrokerで再確認し、対象projectのcopy-ready Action例がcaller repositoryをcheckoutせず空workspaceを走査し得る導入欠陥を特定した。READMEをleast-privilegeな完全workflowへ置換し、immutable checkoutをpreflightより先に要求するpublisher regression testを追加した。
